...
class WDB_PDO
{
     ...
     * @param boolean $_flag
     */
    public function set_mode_debug($_flag = true)
    {
        $this->is_debug = $_flag;
    }
     ...
     */
    function __construct($_dsn, $_user, $_pass, $_charset = '')
    {
        if (empty($_dsn) || empty($_user) || empty($_pass))
        {
            return NULL;
        }
        $result = $this->switch_db($_dsn, $_user, $_pass, $_charset);
     ...
     * @return true|false
     */
     public function switch_db($_dsn, $_user, $_pass, $_charset)
    {
        if (empty($_dsn) || empty($_user) || empty($_pass))
        {
            return false;
        ...
        try
        {
            ...
            if (!empty($_charset))
            {
                $set_charset = $this->execute('SET NAMES ' . $_charset);
     ...
     * @return {object} PDOStatement    PDO申明对象
     */
   ...
   */
    ...
    {
        return $this->wdb_pdo->quote($string);
    }
}
