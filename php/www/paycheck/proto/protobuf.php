...
require_once (dirname(dirname(__FILE__)) . '/proto/base.php');
//引入解析出来的PHP类
 ...
 *
 **/
class Protobuf extends protobuf_socket{
    private $header;
    private $header_buf;
    private $body;
    private $return_body;
    ...
    function __construct($ip,$port,$proto_file_prefix) {
        ...
        require_once (dirname(dirname(__FILE__)) . '/proto/pb_proto_ahero_' . $proto_file_prefix. '.php');

//        require_once('proto/pb_proto_ahero_' . $proto_file_prefix. '.php');
    }
    ...
    public function send_msg($header_name,$header_param,$body_name,$body_param,$body_fmt)
    {
        ...
        $a_return=$this->unpack($this->return,$body_fmt);
        return $a_return;
    }
    public function unpack($return,$body_fmt)
    {
        print_r($return);
        ...
        if($errcode == 0)
        {
            $msg_name=$this->header->msg_name();
            $msg_name=substr($msg_name,strpos($msg_name,'.')+1);
            //如果没有这个类，说明没有返回值 ，则返回空
            if (class_exists($msg_name)) {
